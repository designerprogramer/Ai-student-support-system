...
export default function StudentReminders() {
  ...
  const loadData = async () => {
    try {
      const [complaintData, notificationData] = await Promise.all([
        fetchComplaints(),
        fetchNotifications()
      ])
  ...
  const myComplaints = useMemo(() => {
    ...
    return complaints.filter((c) => Number(c.student) === Number(currentUser.id))
  ...
  const getReminderLabel = (complaint) => {
    const daysRemaining = getDaysRemaining(complaint)
    if (isComplaintClosed(complaint)) return 'Resolved successfully'
  ...
  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center">
        ...
        <span className="ml-3 text-sm text-gray-500 font-medium">Loading notifications...</span>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <DashboardHeader
        ...
        description="Stay updated with real-time status updates on your submitted complaints, assignments, and critical resolution SLAs."
      />
      ...
      <div className="grid gap-8 lg:grid-cols-12">
        ...
        <div className="lg:col-span-7 bg-white rounded-[32px] border border-gray-200/70 p-6 shadow-[0_24px_90px_-40px_rgba(15,23,42,0.25)] flex flex-col">
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-[28px] border border-green-100 bg-[#F5FBF7] p-5 shadow-sm">
            <div>
              <h2 className="text-xl font-bold text-[#0F6A4B] flex items-center gap-2">
                <Bell className="h-5 w-5 text-[#168055]" />
                Notification Feed
              </h2>
              <p className="text-sm text-gray-500 mt-1">Real-time alerts styled for quick scanning.</p>
            </div>
            ...
            {filteredNotifications.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <div className="rounded-full bg-[#F5F9F4] p-5 border border-[#E8F3EA]">
                  <Inbox className="h-9 w-9 text-[#168055]" />
                </div>
        ...
        <div className="lg:col-span-5 bg-white rounded-[32px] border border-gray-200/70 p-6 shadow-[0_24px_90px_-40px_rgba(15,23,42,0.25)] flex flex-col">
          <div className="rounded-[28px] border border-[#E4F4EB] bg-[#F5FBF7] p-5 shadow-sm">
            ...
            <p className="text-sm text-gray-500 mt-1">Track resolution windows with quick summaries.</p>
          </div>
          ...
          <div className="mt-5 space-y-4 max-h-[620px] overflow-y-auto pr-2">
            ...
            {myComplaints.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="mt-4 text-base font-bold text-gray-900">No active complaints</h3>
